...
function handleInput(gameState, playerId, keys, isAI = false) {
	...
	if (gameState.GamePlayLocal) keys.forEach((key) => handleInputLocal(playerId, gameState, key, isAI));
	else
		keys.forEach((key) => {
			if (key === "ArrowUp" || key === "w") movePaddle(gameState, playerId, "up");
			else if (key === "ArrowDown" || key === "s") movePaddle(gameState, playerId, "down");
		});
}

function handleInputLocal(playerId, gameState, key, isAI) {
	if (gameState.aiGame) {
		if (playerId === "p1" && (key === "w" || key === "ArrowUp")) movePaddle(gameState, "p1", "up");
		else if (playerId === "p1" && (key === "ArrowDown" || key === "s")) movePaddle(gameState, "p1", "down");
	} else {
		if (key === "w") movePaddle(gameState, "p1", "up");
		else if (key === "s") movePaddle(gameState, "p1", "down");
	}

	if (isAI || !gameState.aiGame) {
		if (key === "ArrowUp") movePaddle(gameState, "p2", "up");
		else if (key === "ArrowDown") movePaddle(gameState, "p2", "down");
	}
}
...
function getImpactAngle(gameState, impact) {
	let angle;
	if (impact > 0.45) angle = 0.45;
	else if (impact < -0.45) angle = -0.45;
	else angle = impact;
	gameState.speed += 0.05;
	let extra = 0.2;
	if (angle < 0) extra = -0.2;
	return angle + extra;
}
...
function updateBall(gameState) {
	...
	if (gameState.ball.x + ballSizeX <= 0 || gameState.ball.x >= 100) {
		if (gameState.ball.x + ballSizeX <= 0) gameState.score.p2 += 1;
		else gameState.score.p1 += 1;
	...
	if ((gameState.score.p2 === 5 || gameState.score.p1 === 5) && gameState.onGoing) {
		...
		gameState.winnerName = gameState.playersName.player1 || "Villager";
	...
	// (Player 1) left
	if (gameState.ball.x <= paddleWidth && ballCenterY >= gameState.paddles.p1 - HitBoxBuffer && ballCenterY <= gameState.paddles.p1 + paddleHeight) {
	...
	//AI Game
	if (gameState.aiGame) aiMove(gameState, handleInput);
}
...
function updateBall4Players(gameState) {
	...
	if ((gameState.score.p2 === 10 || gameState.score.p1 === 10) && gameState.onGoing) {
		...
		if (gameState.score.p1 < gameState.score.p2) gameState.winnerId = gameState.playerId.p2;
		insertOnDb(gameState);
		gameState.onGoing = false;
		gameState.finished = true;
	}
...
function insertOnDb(gameState) {
	if (gameState.GamePlayLocal || gameState.aiGame) return;
	if (gameState.playerDbId.p1 === null || gameState.playerDbId.p2 === null) return;
	...
	if (gameState.playerDbId.p3 != 0) {
		...
		else insertMatch(gameState.playerDbId.p3, gameState.playerDbId.p4, gameState.playerDbId.p4, gameState.score.p1, gameState.score.p2);
	}
}
