...
module.exports = async function (fastify) {
	...
	fastify.post("/switch-nickname", { preHandler: fastify.authenticate }, async (request, reply) => {
		...
		if (!newNickname) return reply.code(400).send({ error: "Nickname is required" });

		if (newNickname.length > 8) return reply.code(400).send({ error: "Nickname too long" });
	...
	fastify.post("/switch-email", { preHandler: fastify.authenticate }, async (request, reply) => {
		...
		if (!emailRegex.test(newEmail)) return reply.code(400).send({ error: "Invalid email format" });
	...
	fastify.post("/switch-password", { preHandler: fastify.authenticate }, async (request, reply) => {
		...
		if (newPassword.length < 8) return reply.code(400).send({ error: "Password too short" });

		if (!/\d/.test(newPassword)) return reply.code(400).send({ error: "Password needs at least 1 number" });

		if (!/[a-z]/.test(newPassword)) return reply.code(400).send({ error: "Password needs at least lower case" });

		if (!/[A-Z]/.test(newPassword)) return reply.code(400).send({ error: "Password needs at least upper case" });

		if (!/[^A-Za-z0-9]/.test(newPassword)) return reply.code(400).send({ error: "Password needs a special character" });
